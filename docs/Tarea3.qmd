...
create_rt_density_plot <- function(data, condition_name) {
  # Filter out extreme RT values if needed (optional)
  rt_data <- data %>%
    filter(RT_ms > 0 & RT_ms < 2000)  # Keep RTs between 0 and 2000ms
...
rt_density_fast <- create_rt_density_plot(resp_ericsen_rapido, "Fast")
rt_density_slow <- create_rt_density_plot(resp_ericsen_lento, "Slow")

# Display both plots
grid.arrange(rt_density_fast, rt_density_slow, ncol = 1, nrow = 2)

# Alternative version with violin plots for better comparison
create_rt_violin_plot <- function(data, condition_name) {
  # Filter out extreme RT values
  rt_data <- data %>%
...
# Create violin plots
rt_violin_fast <- create_rt_violin_plot(resp_ericsen_rapido, "Fast")
rt_violin_slow <- create_rt_violin_plot(resp_ericsen_lento, "Slow")
