...
# Function to create arrow pattern distribution plot
create_pattern_distribution <- function(data, condition_name) {
  # Get the full data with arrow patterns before filtering responses
  ...
  # Get unique arrow patterns per trial (from Picture events)
...
# Create pattern distribution plots
...
# Display pattern distribution plots
grid.arrange(pattern_fast, pattern_slow, ncol = 1, nrow = 2)

# Function to create density plot with Trial on X-axis for incorrect responses
create_incorrect_density_plot <- function(data, condition_name) {
  # Calculate percentage of incorrect responses per trial and arrow pattern
  density_data <- data %>%
    group_by(Trial, arrow_pattern) %>%
    summarise(
      total = n(),
...
# Create incorrect response plots for both conditions
...
# Display incorrect response plots
